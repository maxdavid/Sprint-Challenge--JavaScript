...
class CubeMaker extends CuboidMaker {
  constructor(attributes) {
    if (!(attributes.length === attributes.width) && !(attributes.length === attributes.height)) {
      console.log(`This isn't a cube!`);
    }
    super(attributes);
  }
  isCube(attributes) { return (attributes.length === attributes.width === attributes.height); }
